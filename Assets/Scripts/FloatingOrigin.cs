...
namespace twoloop
{
    public class FloatingOrigin : MonoBehaviour
    {
        ...
        private void DoRecenter(Vector3 focusPosition)
        {
            ...
            MoveCinemachines(focus, -_focusPosition);
        ...
        private static void MoveTrailRenderers(Vector3 offset)
        {
            ...
            {
                // Store;
                bool wasEmitting = trailRenderers[i].emitting;
                bool wasEnabled = trailRenderers[i].enabled;

                var newPositions = new Vector3[trailRenderers[i].positionCount];
                var trailRendererPositionsCount = trailRenderers[i].GetPositions(newPositions);

                trailRenderers[i].enabled = false;
                trailRenderers[i].emitting = false;

                // -1 bandaid fix to avoid stretching-bug. Don't know a proper solution.
                // Does not 100% fix the issue, there's still a bit of a "hiccup".
                for (int j = 0; j < trailRendererPositionsCount - 1; j++)
                {
                    newPositions[j] += offset;
                }
                trailRenderers[i].SetPositions(newPositions);

                // Restore;
                trailRenderers[i].enabled = wasEnabled;
                trailRenderers[i].emitting = wasEmitting;
            }
        ...
        private static void MoveCinemachines(Transform target, Vector3 offset)
        {
            int vCamCount = CinemachineCore.Instance.VirtualCameraCount;

            for (int i = 0; i < vCamCount; i++)
            {
                CinemachineCore.Instance.GetVirtualCamera(i).OnTargetObjectWarped(target, offset);
            }
        }
    }
}